...
class ScannerWaitTests(unittest.TestCase):
    ...
    def test_prepare_item_inventory_applies_filter_before_sort_rule(self) -> None:
        ...
        scanner._region_capture_match_score = lambda _name: 0.0
    ...
    def test_prepare_item_inventory_uses_mixed_present_filter_and_default_sort_for_presents(self) -> None:
        scanner = self._scanner()
        events: list[str] = []
        scanner._wait = lambda *_args, **_kwargs: True
        scanner._open_item_inventory_filter_panel = lambda **_kwargs: events.append("open:filtermenu_button") or True
        scanner._click_region_capture = lambda name, *, label="", delay=0.0: events.append(f"click:{name}") or True
        scanner._click_region_capture_x_from_y = lambda x_name, y_name, *, label="", delay=0.0: events.append(f"click_mixed:{label}:{x_name}:{y_name}") or True
        scanner._ensure_region_matches_reference = lambda name, **_kwargs: events.append(f"ensure:{name}") or True
        scanner._region_capture_match_score = lambda _name: 0.0
        ...
        self.assertEqual(
            ...
            [
                "open:filtermenu_button",
                "click:filter_reset_button",
                "click_mixed:presents_filter:ooparts_filter:note_filter",
                "click:sort_tab",
                "ensure:sort_rule_check",
                "click:filter_confirm_button",
            ],
    ...
    def test_item_filter_menu_open_accepts_item_sort_reference(self) -> None:
        ...
        scanner._capture = lambda: object()
        scanner._inventory_filter_title_score = lambda _img: 0.0
        scanner._region_capture_match_score = lambda name: 0.72 if name == "sort_rule_check" else 0.0
        scanner._debug = lambda *_args, **_kwargs: None

        self.assertTrue(scanner._wait_for_item_inventory_filter_menu_open(initial_wait=0.0))
    ...
    def test_prepare_item_inventory_switches_from_sort_tab_to_filter_tab(self) -> None:
        scanner = self._scanner()
        events: list[str] = []
        scanner._wait = lambda *_args, **_kwargs: True
        scanner._open_item_inventory_filter_panel = lambda **_kwargs: events.append("open:filtermenu_button") or True
        scanner._click_region_capture = lambda name, *, label="", delay=0.0: events.append(f"click:{name}") or True
        scanner._ensure_region_matches_reference = lambda name, **_kwargs: events.append(f"ensure:{name}") or True
        scanner._region_capture_match_score = lambda name: 0.82 if name == "sort_rule_check" else 0.0
