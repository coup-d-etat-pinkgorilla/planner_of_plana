...
class InventoryCandidateContext {
  const InventoryCandidateContext({
...
class _InventoryPageState extends State<InventoryPage> {
  ...
  Future<void> _loadCatalog() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _loading = false;
          _catalogError = 'Could not load inventory catalog: $error';
        });
      }
    }
  }
  ...
  void _syncEditors() {
    ...
    for (final entry
        in _repositoryState?.inventory.entries ??
            const <Map<String, dynamic>>[]) {
      quantities[(entry['item_id'] ?? entry['key']) as String] =
          entry['quantity'] as String?;
    }
    for (final item in _catalog) {
      _editors.putIfAbsent(item.resourceKey, TextEditingController.new).text =
          quantities[item.resourceKey] ?? '';
    }
    _dirtyKeys.clear();
  }
  ...
  Map<String, dynamic> _inventoryWire() {
    ...
    final existingByKey = <String, Map<String, dynamic>>{
      for (final entry
          in _repositoryState?.inventory.entries ??
              const <Map<String, dynamic>>[])
        (entry['item_id'] ?? entry['key']) as String: Map<String, dynamic>.from(
          entry,
        ),
    };
    final entries = <Map<String, dynamic>>[
      for (final entry
          in _repositoryState?.inventory.entries ??
              const <Map<String, dynamic>>[])
        if (!catalogKeys.contains(entry['item_id'] ?? entry['key']))
    ...
    for (final item in _catalog) {
      ...
      if (!canonical.hasMatch(quantity)) {
        throw FormatException(
          '${item.displayName}: enter 0 or a non-negative integer.',
        );
      }
      entries.add({
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      final revision = await repository.saveRepositoryInventory(
        profile.id,
        snapshot,
        state.revision,
      ...
      setState(() {
        ...
        _shortages = null;
        _message = 'Saved complete inventory bucket at revision $revision.';
      });
      _syncEditors();
    } catch (error) {
      if (mounted) {
        setState(
          () => _message =
              'Save failed; your draft was kept. Reload before retrying if the revision changed: $error',
        );
      }
    } finally {
  ...
  Future<void> _calculateShortages() async {
    final state = _repositoryState;
    if (state == null) {
      setState(() => _message = 'Select a profile to analyze its saved plan.');
      return;
    }
    if (state.goals.isEmpty) {
      setState(
        () => _message =
            'This profile has no saved goals. Open Planning to save a plan first.',
      );
      return;
    }
    setState(() {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          _loading = false;
          _message = 'Could not calculate shortages: $error';
        });
      }
    }
  }
  ...
  Future<void> _approveCandidate() async {
    ...
    try {
      final approved =
          context.candidate.reviewRequired && !context.candidate.approved
          ? await scanner.reviewScannerCandidate(
              context.session,
              context.candidate,
              context.candidate.payload,
              approve: true,
              reason: 'approved_in_inventory_page',
            )
          : context.candidate;
      await scanner.commitScannerCandidate(
        context.session,
        approved,
        profileId: profile.id,
        expectedRepositoryRevision: state.revision,
        idempotencyKey:
            'inventory-candidate-${approved.id}-${approved.revision}',
      );
      await _reload();
      ...
      if (mounted) {
        setState(
          () =>
              _message = 'Candidate commit failed; comparison was kept: $error',
        );
      }
    } finally {
      if (mounted) setState(() => _saving = false);
  ...
  List<InventoryCatalogEntry> get _visible {
    ...
    final catalogRank = {
      for (var index = 0; index < _catalog.length; index++)
        _catalog[index].resourceKey: index,
    };
    final shortageByKey = {
      for (final row in _shortages?.rows ?? const <InventoryShortageRow>[])
        row.resourceKey: row,
    };
    final values = _catalog.where((item) {
      final modeMatches =
          _mode == 'all' ||
          (_mode == 'equipment'
              ? item.profileId == 'equipment'
              : item.profileId != 'equipment');
      return modeMatches &&
          (_category == null || item.category == _category) &&
          item.matches(query);
    }).toList();
    int quantity(InventoryCatalogEntry item) =>
        int.tryParse(_editors[item.resourceKey]?.text ?? '') ?? -1;
    values.sort((a, b) {
      final primary = switch (_sort) {
        'name' => a.displayName.toLowerCase().compareTo(
          b.displayName.toLowerCase(),
        ),
        'quantity' => quantity(b).compareTo(quantity(a)),
        'shortage' => (shortageByKey[b.resourceKey]?.shortage ?? -1).compareTo(
          shortageByKey[a.resourceKey]?.shortage ?? -1,
        ),
  ...
  @override
  Widget build(BuildContext context) => ValueListenableBuilder<AppServiceState>(
    ...
    builder: (context, serviceState, _) {
      ...
      final categories = _catalog.map((item) => item.category).toSet().toList()
        ..sort();
      final shortages = {
        for (final row in _shortages?.rows ?? const <InventoryShortageRow>[])
          row.resourceKey: row,
      ...
      return ColoredBox(
        ...
        child: ListView(
          ...
          children: [
            ...
            DiagonalSection(
              child: Padding(
                padding: const EdgeInsets.fromLTRB(18, 16, 34, 18),
                child: Wrap(
                  spacing: 12,
                  runSpacing: 10,
                  ...
                  children: [
                    ...
                    SegmentedButton<bool>(
                      segments: const [
                        ButtonSegment(value: false, label: Text('Owned items')),
                        ButtonSegment(
                          value: true,
                          label: Text('Plan shortages'),
                        ),
                      ],
                      selected: {_showShortages},
                      onSelectionChanged: (value) =>
                          setState(() => _showShortages = value.first),
                    ),
                    FilledButton.icon(
                      onPressed: connected && !_loading
                          ? _calculateShortages
                          : null,
                      icon: const Icon(Icons.analytics_outlined),
                      label: const Text('Analyze saved plan'),
                    ),
                    OutlinedButton.icon(
            ...
            const SizedBox(height: 12),
            if (widget.candidateContext != null) ...[
              _CandidatePanel(
                context: widget.candidateContext!,
                current: _repositoryState?.inventory,
                busy: _saving,
                onHold: () => setState(
                  () => _message =
                      'Candidate held; repository inventory was not changed.',
                ),
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    onChanged: (value) =>
                        setState(() => _sort = value ?? 'catalog'),
                  ),
                ),
                FilledButton.icon(
                  onPressed: connected && !_saving && _repositoryState != null
                      ? _save
                      : null,
                  icon: const Icon(Icons.save_outlined),
                  label: Text(_saving ? 'Saving…' : 'Save inventory'),
                ),
                TextButton(
                  onPressed: _profile == null ? _loadCatalog : _reload,
            ...
            if (_repositoryState != null &&
                _repositoryState!.inventory.entries.isEmpty)
              const Padding(
                padding: EdgeInsets.only(bottom: 10),
                child: Text(
                  'Inventory snapshot is empty. Catalog-only rows remain Unknown until an explicit quantity is saved.',
            ...
            if (!connected)
              const _InventoryNotice(
                ...
                body: 'Reconnect to load catalog, inventory, and shortages.',
              )
            else if (_loading)
              const Center(
                child: Padding(
              ...
              const _InventoryNotice(
                ...
                title: 'Empty catalog',
                body: 'No inventory metadata was returned.',
              )
            else if (_showShortages && _shortages == null)
              const _InventoryNotice(
                icon: Icons.analytics_outlined,
                title: 'No shortage result',
                body:
                    'Analyze the selected profile’s saved plan and inventory.',
              )
            else if (_visible.isEmpty)
              const _InventoryNotice(
                icon: Icons.filter_alt_off,
                title: 'No matching items',
                body: 'Change the search or category filter.',
              )
            else
              for (final item in _visible)
                Padding(
                  ...
                  child: _InventoryRow(
                    item: item,
                    controller: _editors[item.resourceKey]!,
                    shortage: shortages[item.resourceKey],
                    showShortage: _showShortages,
                    enabled: _repositoryState != null && !_saving,
                    onChanged: () => _dirtyKeys.add(item.resourceKey),
                  ),
                ),
            if (_shortages?.warnings.isNotEmpty == true) ...[
              const SizedBox(height: 8),
              Text(
                _shortages!.warnings.join('\n'),
                style: const TextStyle(color: AppColors.warning),
              ),
            ],
          ],
...
class _InventoryRow extends StatelessWidget {
  const _InventoryRow({
    ...
    required this.showShortage,
    required this.enabled,
    required this.onChanged,
  });
  final InventoryCatalogEntry item;
  final TextEditingController controller;
  final InventoryShortageRow? shortage;
      ...
      child: Wrap(
        ...
        children: [
          SizedBox(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  '${item.category} · ${item.profileId} · #${item.orderIndex}',
                  style: const TextStyle(
                    color: AppColors.textMuted,
                    fontSize: 12,
                  ),
          ...
          SizedBox(
            width: 150,
            child: TextField(
              key: ValueKey('inventory-quantity-${item.resourceKey}'),
          ...
          if (showShortage) ...[
            _Metric(
              label: 'Required',
              value: shortage?.requiredAmount.toString() ?? '—',
            ),
            _Metric(
              label: 'Owned',
              value: shortage?.owned?.toString() ?? 'Unknown',
            ),
            _Metric(
              label: 'Shortage',
              value: shortage?.shortage?.toString() ?? 'Unknown',
            ),
            SizedBox(
              width: 220,
              child: Text(
                shortage == null
                    ? 'Not required by saved plan'
                    : shortage!.affectedStudentIds.isEmpty
                    ? 'No affected students'
                    : 'Students: ${shortage!.affectedStudentIds.join(', ')}',
                maxLines: 2,
                overflow: TextOverflow.ellipsis,
              ),
            ),
...
class _InventoryNotice extends StatelessWidget {
  ...
  Widget build(BuildContext context) => DiagonalSection(
    child: Padding(
      padding: const EdgeInsets.fromLTRB(18, 22, 34, 22),
      child: Row(
        children: [
          Icon(icon, size: 34),
...
class _CandidatePanel extends StatelessWidget {
  ...
  final InventoryCandidateContext context;
  final RepositoryInventoryState? current;
  final bool busy;
  ...
  Widget build(BuildContext buildContext) {
    final candidateEntries =
        context.candidate.payload['entries'] as List? ?? const [];
    final currentValues = {
      for (final item in current?.entries ?? const <Map<String, dynamic>>[])
        item['item_id'] ?? item['key']: item['quantity'],
    };
    final candidateValues = {
      for (final item in candidateEntries.whereType<Map>())
        item['item_id'] ?? item['key']: item['quantity'],
    };
    final currentKeys = currentValues.keys.toSet();
    final candidateKeys = candidateValues.keys.toSet();
    final added = candidateKeys.difference(currentKeys).length;
    final missing = currentKeys.difference(candidateKeys).length;
    final changed = candidateKeys
        .intersection(currentKeys)
        .where((key) => candidateValues[key] != currentValues[key])
        .length;
    final unknown = candidateValues.values
    ...
    return DiagonalSection(
      child: Padding(
        ...
        child: Wrap(
          ...
          children: [
            Text(
              'Scanner candidate · added $added / changed $changed / missing $missing / unknown $unknown',
            ),
            if (context.candidate.reviewRequired)
              const Chip(label: Text('Review required')),
            const Text(
              'Missing rows stay unknown; they are never auto-filled with zero.',
              style: TextStyle(color: AppColors.textMuted),
            ),
            OutlinedButton(
