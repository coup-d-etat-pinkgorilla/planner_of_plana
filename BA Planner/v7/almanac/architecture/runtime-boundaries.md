...
planning slice의 실행 경계는 JSON Lines stdin/stdout Python process와 Dart process
client입니다. 기본 UI 실행은 mock을 유지하고 compile-time define으로만 실제
`ProcessAppService`를 선택합니다. process 진단은 stderr로 분리하며 Flutter는 Python
모듈을 직접 import하지 않습니다. [@frontend-process-service] [@backend-process]
