...
아래 표는 P6의 기능 범위를 고정한다. 각 기능을 사용자가 어떤 순서로 작동시키는지와
어떤 지점에서 다른 탭으로 이어지는지는
[Frontend Section Direction and User Flows](../design/frontend-section-direction-and-user-flows)를
따른다. 해당 문서는 레이아웃 시안이 아니라 사용자 검수 전의 흐름 가설이다.
...
P6은 통합 베타 완료 기준이다. 전술대항전의 로비 스캔, 상대 방어 이력, 결정론적 통계,
방어 변경 감지, 예상 방어덱과 공유 분석은
[P7-P13 Tactical Challenge Backend Workflow](p7-p13-tactical-backend-workflow)에서
별도 순차 단계로 관리한다. P7 production 구현은 P6 완료 판정 전에는 시작하지 않는다.

v6 전체 기능 parity 감사, scanner 해상도·게임 업데이트 안정화, 성능, 백업·복구,
접근성, installer, 서명, 자동 업데이트와 release 운영은 이 후속 전술 워크플로와도
구분된 안정화 단계로 관리한다.
