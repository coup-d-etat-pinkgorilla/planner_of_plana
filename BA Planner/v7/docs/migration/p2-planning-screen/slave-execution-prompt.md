...
마스터가 같은 신뢰 가능한 Wi-Fi/LAN에서 결과 수신 래퍼를 시작했다면 다음 단일
명령으로 마스터를 자동 발견하여 네 파일을 무선 전송하십시오. IP, port 또는 token을
사용자에게 요구하지 마십시오.
...
송신 래퍼는 UDP discovery nonce를 사용해 일회용 token을 메모리로만 받습니다.
token은 화면, 파일, 로그, output.md 또는 최종 보고에 기록하지 마십시오. 마스터 수신기가
`WIRELESS_HANDOFF_RECEIVED`를 확인한 경우에만 무선 전송을 완료로 표시하십시오.
