...
- 실행 환경: Windows, Python 3.11, Flutter Windows
...
P5 구현 전에 현재 tree에서 다음 상태를 재현한다.
...
4. Flutter 전체 43 tests와 `flutter analyze` 통과
5. Windows release build, `codealmanac validate`, `git diff --check` 통과
6. 실제 Dart `ProcessAppService`가 서로 다른 Python child process 두 개를 사용해 같은
   temporary storage root에서 profile ID, display name, revision 3과 canonical goal을 복원
7. confirmed current, inventory와 goal은 typed repository state이며 Flutter가 저장 파일이나
   SQLite를 직접 읽지 않음
8. planning/repository runtime에서 `../v6`, scanner 또는 Qt/Tk/PySide6 import 0건
9. 실제 사용자 profile/DB/home storage를 test에서 사용하지 않음

승인 baseline의 수치나 동작이 다르면 누락된 선행 patch를 임의 재구성하거나 P4 의미를
바꾸어 맞추지 않는다. 현재 commit/diff, 명령, 기대값과 실제값을 기록하고 안전하게 계속할
수 없으면 구현을 시작하지 말고 `TASK_OUTPUT_BLOCKED`로 반환한다.
...
- P4 baseline gate의 실제 fixture/test 수와 restart E2E 결과
...
- 실제 Dart↔Python event E2E와 process/worker/subscription cleanup
...
필수 자동 검증 하나라도 실패하거나 student/inventory 실제 adapter 중 하나가 fake/placeholder로
남으면 `COMPLETED`로 보고하지 않는다.
