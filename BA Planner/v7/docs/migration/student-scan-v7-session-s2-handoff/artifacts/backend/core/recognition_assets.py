...
class RecognitionAssetCatalog:
    ...
    def load(self) -> dict[str, Any]:
        ...
        combined_assets = list(value["assets"])
        for name in self.AUXILIARY_MANIFESTS:
            path = self.root / name
            if not path.is_file():
                continue
            try:
                auxiliary = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, ValueError) as exc:
                raise ScannerError("asset_manifest_invalid", str(exc)) from exc
            if (
                not isinstance(auxiliary, dict)
                or auxiliary.get("version") != self.VERSION
                or not isinstance(auxiliary.get("assets"), list)
            ):
                raise ScannerError("asset_version_mismatch", f"{name} version must be {self.VERSION}")
            combined_assets.extend(auxiliary["assets"])
        value["assets"] = combined_assets
